...
def prepare_facebank_retina(conf, model, retina_face, tta = True):
    model.eval()
    embeddings =  []
    names = ['Unknown']
    tmp_count = 0
            ...
            for file in path.iterdir():
                if not file.is_file():
                    ...
                    try:
                        ...
                        continue
                    if img.size != (112, 112):
                        img2Path = './'+file.__str__()
                        img2Path = img2Path.replace('\\', '/')
                        img2 = cv2.imread(img2Path)
                        # print(img2Path)
                        tmp_count += 1
                        img = retina_face.align(img2)
                        #img = mtcnn.align(img)
                    with torch.no_grad():
                        if tta:
                            mirror = trans.functional.hflip(img)
    ...
    embeddings = torch.cat(embeddings)
    names = np.array(names)
    print('图库人个数:{}，共人脸{}张'.format(tmp_count_all, tmp_count))
    torch.save(embeddings, str(conf.facebank_path/'facebank.pth'))
    np.save(conf.facebank_path/'names', names)
    return embeddings, names

def prepare_facebank_retina_coll(conf, model, retina_face, tta = True):
    model.eval()
    embeddings = []
    names = ['Unknown']
    tmp_count = 0

    for file in conf.facebank_path.iterdir():
        embs = []
        if not file.is_file():
            continue
    ...
    embeddings = torch.cat(embeddings)
    names = np.array(names)
    print('图库人脸{}张'.format(tmp_count))
    print('faces embeddings: {}'.format(embeddings.shape))
    torch.save(embeddings, str(conf.facebank_path / 'facebank.pth'))
    np.save(conf.facebank_path / 'names', names)
