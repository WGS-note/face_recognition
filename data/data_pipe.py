...
import numpy as np
import bcolz # 一个高压缩率，时序数据存储python 库
import pickle # 将Python对象层次结构转换为字节流的过程
import torch
import mxnet as mx # 亚马逊深度学习库
...
def get_train_dataset(imgs_folder):
    train_transform = trans.Compose([
        #trans.CenterCrop(113), # 依据跟定的size,从中心进行裁剪
        trans.RandomHorizontalFlip(), # 随机水平翻转
        trans.ToTensor(), # numpy转tensor张量
        trans.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5]) # 对图像进行标准化
    ])
    ds = ImageFolder(imgs_folder, train_transform) # 参数：图片路径；翻转后的图片
    class_num = ds[-1][1] + 1
    return ds, class_num
